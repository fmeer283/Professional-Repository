             ...
             implements BSTInterface<T>
{
  ...
  public int countLess2(T maxValue) 
  {
	  return recCountLess(maxValue, root);
  }
  
  public int recCountLess(T maxValue, BSTNode<T> tree)
  {
	  int toReturn = 0;
	  if (tree == null)    
	      return toReturn;
	  else if (maxValue.compareTo(tree.getInfo()) == 0)
		  toReturn++;
	  
	return recCountLess(maxValue, tree.getLeft()) + 
			recCountLess(maxValue, tree.getRight()) + toReturn;
  }
  // Iteration
	  ...
	  int toReturn = 0;
	    if (root != null)
	    {
	      ...
	      while (!hold.isEmpty())
	      {
	        currNode = hold.top();
	        hold.pop();
	        if (currNode.getInfo().compareTo(maxValue) == 0) {
	        	toReturn++;
	        }
